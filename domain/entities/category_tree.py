...
class CategoryTree:
    ...
    def _build_tree(self) -> CategoryNode:
        ...
        unknown_category = Category(
            UNKNOWN_CATEGORY_ID, "Unknown", "Unknown categories", ALL_CATEGORY_ID
        )
    ...
    def _filter_transactions_by_date(
        self, transactions: List[Transaction], from_date: Optional[str], to_date: Optional[str]
    ) -> List[Transaction]:
        ...
        for tx in transactions:
            ...
            if to_date and include_tx:
                ...
                if to_datetime:
                    ...
                    if (
                        to_datetime.hour == 0
                        and to_datetime.minute == 0
                        and to_datetime.second == 0
                    ):
        ...
        for tx in filtered_transactions:
            ...
            if converter and user_currency:
                ...
                converted_amount = converter.convert(
                    amount_major, tx.currency, user_currency, tx.date
                )
            else:
        ...
        for tx in self.filtered_transactions:
            if tx.category is None or tx.category not in self.categories:
                if tx.description not in uncategorized:
                    uncategorized[tx.description] = {"count": 0, "total": 0.0, "tx_ids": []}
                uncategorized[tx.description]["count"] += 1
                uncategorized[tx.description]["total"] += to_major_units_float(
    ...
    def print_tree(
        self, node: Optional[CategoryNode] = None, indent: int = 0, total: Optional[float] = None
    ) -> None:
        if node is None:
            node = self.root
