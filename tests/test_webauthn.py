...
class TestRegisterOptions:
    ...
    def test_returns_valid_json(self, authenticated_client, mock_webauthn_services):
        with patch('presentation.web.blueprints.webauthn.generate_registration_options') as mock_gen, \
             patch('presentation.web.blueprints.webauthn.options_to_json') as mock_to_json:
...
class TestRegisterVerify:
    ...
    def test_first_passkey_calls_setup_encryption(self, authenticated_client, mock_webauthn_services):
        ...
        with patch('presentation.web.blueprints.webauthn.verify_registration_response') as mock_verify:
            ...
            response = authenticated_client.post(
                ...
                content_type='application/json',
                data=json.dumps({
                    'credential': {'id': 'abc', 'rawId': 'abc', 'response': {
                        'attestationObject': 'YQ', 'clientDataJSON': 'YQ',
                    }, 'type': 'public-key'},
                    'kek': 'kek_value',
                    'prfSalt': 'salt_value',
    ...
    def test_additional_passkey_calls_add_passkey_wrapper(self, authenticated_client, mock_webauthn_services):
    ...
    def test_additional_passkey_without_existing_dek_rejected(self, authenticated_client, mock_webauthn_services):
...
class TestAuthenticateOptions:
    ...
    def test_returns_options_with_prf_salts(self, authenticated_client, mock_webauthn_services):
        ...
        with patch('presentation.web.blueprints.webauthn.generate_authentication_options') as mock_gen, \
             patch('presentation.web.blueprints.webauthn.options_to_json') as mock_to_json:
            mock_to_json.return_value = json.dumps({
                'challenge': 'dGVzdA',
                'rpId': 'localhost',
...
class TestDismissEncryptionBanner:
    ...
    def test_preserves_existing_browser_settings(self, authenticated_client, mock_webauthn_services):
...
class TestMigrateEncrypt:
    ...
    def test_encrypts_google_token_even_with_zero_transactions(self, unlocked_client, mock_webauthn_services):
