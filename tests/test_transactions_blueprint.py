...
def mock_services():
    ...
    mock_user_settings_service.validate_currency.side_effect = (
        lambda c: c in ['JPY', 'USD', 'EUR']
    )
    ...
    patches = {
        ...
        'cache_mgr': patch(
            'presentation.web.blueprints.transactions.get_cache_manager',
...
class TestTransactionsBlueprint:
    ...
    def test_update_transaction_missing_id(self, authenticated_client, mock_services):
        ...
        response = authenticated_client.post(
            '/update-transaction',
            json={'tx_id': '', 'date': '2025-01-01', 'amount': '1000',
                  'description': 'Test', 'comment': '', 'currency': 'JPY'},
        )
    ...
    def test_update_transaction_invalid_currency(self, authenticated_client, mock_services):
        ...
        response = authenticated_client.post(
            '/update-transaction',
            json={'tx_id': 'tx1', 'date': '2025-01-01', 'amount': '1000',
                  'description': 'Test', 'comment': '', 'currency': 'INVALID'},
        )
        assert response.status_code == 400
        data = response.get_json()
        assert data['success'] is False
    ...
    def test_add_transaction_invalid_currency(self, authenticated_client, mock_services):
        ...
        response = authenticated_client.post(
            '/add-transaction',
            json={'date': '2025-01-01', 'amount': '1000',
                  'description': 'Lunch', 'category': '',
                  'comment': '', 'currency': 'XYZ'},
        )
        assert response.status_code == 400
        data = response.get_json()
        assert data['success'] is False
    ...
    def test_update_transaction_success(self, authenticated_client, mock_services):
        ...
        response = authenticated_client.post(
            '/update-transaction',
            json={'tx_id': 'tx1', 'date': '2025-01-01', 'amount': '1000',
                  'description': 'Updated', 'comment': '', 'currency': 'JPY'},
        )
        assert response.status_code == 200
        data = response.get_json()
        assert data['success'] is True
