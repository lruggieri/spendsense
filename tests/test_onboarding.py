...
class TestOnboardingStep4:
    ...
    def _mock_services(self):
        ...
        with patch('presentation.web.blueprints.onboarding.get_user_settings_service') as mock_settings, \
             patch('presentation.web.blueprints.onboarding.get_fetcher_service') as mock_fetcher, \
             patch('presentation.web.blueprints.onboarding.get_category_service') as mock_category, \
             patch('presentation.web.blueprints.onboarding.get_pattern_service') as mock_pattern, \
             patch('presentation.web.blueprints.onboarding.get_encryption_service') as mock_enc:

            # settings service returns in-progress onboarding at step 4
            settings_svc = MagicMock()
            settings_obj = MagicMock()
            settings_obj.browser_settings = {'onboarding_step': 4, 'onboarding_started_at': '2025-01-01'}
