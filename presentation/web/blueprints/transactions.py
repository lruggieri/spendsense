...
def review():
    ...
    transaction_source = request.args.get(
        "transaction_source", ""
    ...
    logger.debug(
        f"[REVIEW] classify_transactions() took {(time.time() - t1) * 1000:.2f}ms for {len(tx_dict)} txs"
    )
    ...
    logger.debug(
        f"[REVIEW] filter returned {len(transactions)} transactions in {(time.time() - t2) * 1000:.2f}ms"
    )
    ...
    total_amount = round(
        sum(
            converter.convert(
                to_major_units_float(tx.amount, tx.currency), tx.currency, default_currency, tx.date
            )
...
def assign_similarity():
    ...
    for key, value in request.form.items():
        if key.startswith("category_"):
            ...
            logger.debug(
                f"Transaction {tx_id} current category: '{tx.category}', source: '{tx.category_source}'"
            )
            ...
            if tx.category != value or (
                tx.category_source == CategorySource.SIMILARITY and value != ""
            ):
...
def investigate_similarity(tx_id):
    ...
    if tx.category_source != CategorySource.SIMILARITY:
        return (
            jsonify({"success": False, "error": "Transaction was not categorized via similarity"}),
            400,
        )
    ...
    response = jsonify(
        {
            ...
            "transaction": {
                ...
                "category_name": (
                    categories.get(tx.category).name if tx.category in categories else "Unknown"
                ),
            },
...
def add_transaction():
    ...
    if not all([date, amount, description]):
        return (
            jsonify({"success": False, "error": "Date, amount, and description are required"}),
            400,
        )
