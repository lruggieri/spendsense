...
from presentation.web.utils import (
    ...
    invalidate_service_cache, tree_to_dict, load_and_classify
)
...
def groups():
    ...
    if group_id:
        ...
        if selected_group:
            ...
            if sort_by == 'amount':
                transactions = sorted(classified_transactions, key=lambda tx: int(tx.amount), reverse=True)
            ...
            tree.calculate_expenses(classified_transactions, None, None, default_currency, converter)
...
def api_add_group_to_transaction():
    ...
    try:
        ...
        if success:
            ...
            return jsonify({'success': False, 'error': error or 'Transaction not found or group already added'})
...
def api_remove_group_from_transaction():
    ...
    try:
        ...
        if success:
            ...
            return jsonify({'success': False, 'error': error or 'Transaction not found or group not present'})
