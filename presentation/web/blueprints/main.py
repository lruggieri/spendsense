...
from presentation.web.utils import (
    ...
    get_classification_service, build_category_tree_data, load_and_classify
)
...
def service_worker():
    ...
    response = make_response(
        current_app.send_static_file('service-worker.js')
    )
...
def charts():
    ...
    tree_data = build_category_tree_data(category_service, user_settings_service, classified_txs, from_date, to_date)
        ...
        if sum(monthly_amounts) > 0:
            ...
            for subcat, depth in categories:
                ...
                if root_parent == cat.id and subcat.id != cat.id:
                    ...
                    sub_monthly_amounts = []
                    for month in sorted_months:
                        total = sum(monthly_data[month].get(cat_id, 0) for cat_id in sub_descendant_ids)
