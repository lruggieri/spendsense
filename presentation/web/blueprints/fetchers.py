...
from presentation.web.extensions import get_session_datasource, get_credentials_loader_instance
from presentation.web.utils import refresh_google_token_if_needed, get_fetcher_service, get_user_settings_service
...
def _fetch_email_by_id(email_id: str):
    ...
    try:
        ...
        logger.debug(f"Successfully fetched message {email_id}, has payload: {'payload' in message}")
        ...
        if message and 'payload' in message:
            ...
            if 'parts' in payload:
                ...
                for i, part in enumerate(payload['parts']):
                    logger.debug(f"Part {i}: mimeType={part.get('mimeType')}, has_body_data={'data' in part.get('body', {})}")
...
def fetchers():
    ...
    for f in fetchers_data:
        ...
        fetchers_list.append({
            ...
            'from_emails_display': ', '.join(from_emails[:2]) + ('...' if len(from_emails) > 2 else ''),
...
def api_test_fetcher():
    ...
    try:
        ...
        if not all_email_texts:
            return jsonify({
                'success': False,
                'error': 'No valid email examples found'
            }), 400
