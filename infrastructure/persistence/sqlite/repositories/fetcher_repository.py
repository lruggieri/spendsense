...
class SQLiteFetcherDataSource(FetcherRepository):
    ...
    def create_fetcher(self, fetcher: Fetcher) -> bool:
        ...
        try:
            ...
            created_at = fetcher.created_at.isoformat() if fetcher.created_at else datetime.now(timezone.utc).isoformat()
            updated_at = fetcher.updated_at.isoformat() if fetcher.updated_at else datetime.now(timezone.utc).isoformat()
            ...
            conn.commit()
            logger.info(f"Created fetcher {fetcher.id} ({fetcher.name}) v{version} for user {self.user_id}")
            return True
    ...
    def create_new_version(self, old_fetcher_id: str, new_fetcher: Fetcher) -> Optional[Fetcher]:
        ...
        try:
            ...
            conn.commit()
            logger.info(f"Created new version {new_version} of fetcher group {group_id} (new id: {new_fetcher.id})")
    ...
    def _row_to_fetcher(self, row) -> Fetcher:
        ...
        Returns:
            Fetcher entity
        """
        (id, user_id, name, from_emails_json, subject_filter,
         amount_pattern, merchant_pattern, currency_pattern,
         default_currency, negate_amount, enabled, created_at_str, updated_at_str,
         group_id, version) = row
