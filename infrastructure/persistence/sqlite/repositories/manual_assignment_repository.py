...
class SQLiteManualAssignmentDataSource(ManualAssignmentRepository):
    ...
    def get_assignments(self) -> Dict[str, str]:
        ...
        cursor.execute('SELECT tx_id, category_id FROM manual_assignments WHERE user_id = ?', (self.user_id,))
    ...
    def remove_assignment(self, tx_id: str) -> bool:
        ...
        try:
            cursor.execute('DELETE FROM manual_assignments WHERE tx_id = ? AND user_id = ?', (tx_id, self.user_id))
            conn.commit()
    ...
    def get_assignment(self, tx_id: str) -> Optional[str]:
        ...
        cursor.execute('SELECT category_id FROM manual_assignments WHERE tx_id = ? AND user_id = ?', (tx_id, self.user_id))
        row = cursor.fetchone()
    ...
    def has_assignment(self, tx_id: str) -> bool:
        ...
        cursor.execute('SELECT 1 FROM manual_assignments WHERE tx_id = ? AND user_id = ? LIMIT 1', (tx_id, self.user_id))
